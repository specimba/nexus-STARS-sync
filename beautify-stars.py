#!/usr/bin/env python3
"""
beautify-stars.py v2.0 — Transforms the starred-generated README into a beautiful page.

Usage:
    python beautify-stars.py                  # In-place beautify README.md
    python beautify-stars.py -i stars.md -o pretty.md  # Custom paths
    python beautify-stars.py --force --verbose          # Re-beautify with debug output
"""
...
LANG_EMOJI = {
    "Bicep": "🏗️", "C": "🔧", "C#": "🟪", "C++": "⚡", "Cuda": "🟢",
    "CSS": "🎨", "Dart": "🎯", "Dockerfile": "🐳", "Elixir": "🧪",
    "Erlang": "📡", "Go": "🐹", "HCL": "☁️", "HTML": "🌐", "Haskell": "📐",
    "Java": "☕", "JavaScript": "✨", "Jupyter Notebook": "📓", "Kotlin": "🟣",
    "Lua": "🌙", "Makefile": "🔨", "Markdown": "📝", "Nix": "❄️",
    "Objective-C": "🍎", "Others": "📦", "PDDL": "🧩", "Perl": "🐪",
    "PHP": "🐘", "PowerShell": "💠", "Python": "🐍", "R": "📊",
    "Ruby": "💎", "Rust": "🦀", "SCSS": "🎨", "Scala": "🔴", "Shell": "🐚",
    "Swift": "🦅", "TSQL": "🗄️", "TeX": "📄", "TypeScript": "🔷",
    "Vue": "💚", "Zig": "⚡",
}

CATEGORIES = [
    ("🧠 AI & Machine Learning", ["Python", "Jupyter Notebook", "Cuda", "R"]),
    ("🌐 Web Development", ["JavaScript", "TypeScript", "HTML", "CSS", "Vue", "SCSS", "Dart"]),
    ("⚙️ Systems Programming", ["C", "C++", "Rust", "Go", "Shell", "Makefile", "Dockerfile", "HCL", "Nix"]),
    ("📱 App Development", ["Swift", "Kotlin", "Java", "Objective-C", "C#"]),
    ("🔧 Infrastructure", ["PowerShell", "TSQL", "Bicep", "PDDL"]),
    ("📚 Documentation", ["Markdown", "TeX", "Others"]),
    ("🧪 Experimental", ["Elixir", "Erlang", "Haskell", "Lua", "Perl", "PHP", "Ruby", "Scala", "Zig"]),
]
...
def parse_stars_md(content: str) -> dict[str, list[dict]]:
    """Parse the starred-generated README into structured data."""
...
def count_stars(languages: dict[str, list[dict]]) -> int:
    return sum(len(repos) for repos in languages.values())


def truncate_desc(desc: str, max_len: int = 90) -> str:
    if len(desc) <= max_len:
        return desc
...
def make_pct(value: int, total: int) -> str:
    if total == 0:
        return "0.0%"
    return f"{value / total * 100:.1f}%"


def build_header(total_stars: int, lang_count: int) -> str:
...
def build_stats_overview(languages: dict[str, list[dict]]) -> str:
...
def build_toc(languages: dict[str, list[dict]]) -> str:
    total = count_stars(languages)
    ...
    lines = [
        ...
        "### 📑 Browse by Language\n",
        "<details open>",
        f"<summary><b>Languages A–Z</b> · {total} total</summary>\n",
    ]
...
def build_highlighted_repos(languages: dict[str, list[dict]]) -> str:
    lines = [
        "---\n",
        "### 🏆 Highlighted Repos\n",
        "<details>",
...
def build_language_section(lang: str, repos: list[dict]) -> str:
    emoji = LANG_EMOJI.get(lang, "📄")
    lines = [
        f"### {emoji} {lang}",
        "",
...
def is_already_beautified(content: str) -> bool:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="beautify-stars",
        description="Transform starred-generated README into a beautiful page.",
        epilog="""\
Examples:
  %(prog)s                              # Beautify README.md in-place
  %(prog)s -i stars.md -o pretty.md     # Custom input/output
  %(prog)s --force --verbose            # Re-beautify with debug output
""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("-V", "--version", action="version", version=f"%(prog)s {VERSION}")
    parser.add_argument("-i", "--input", default="README.md", help="Input file (default: README.md)")
    parser.add_argument("-o", "--output", default=None, help="Output file (default: same as input)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not input_path.exists():
        ...
        log.error("Run `starred` first to generate the README.")
        return 1
    ...
    log.debug("Parsing starred README...")
    languages = parse_stars_md(content)

    if not languages:
        log.error("No language sections found in input. Is this a starred-generated README?")
        return 1
